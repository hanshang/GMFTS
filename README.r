...
# 22_Gender_specific_point_forecasts_reconciliation.R: reconcile multivariate point forecasts using the bottom-up, optimal combination, and MinT methods before calculating errors.
# 23_Gender_specific_PI_function.R: define functions used to construct multivariate prediction intervals following the gender-specific hierarchical structure.
# 24_Gender_specific_PI_base.R: calculate interval scores for the multivariate interval predictions following the gender-specific hierarchical structure.
# 25_Gender_specific_bottom_up_reconciliation.R: calculate reconciled interval scores for the multivariate forecasting method with the bottom-up method following the gender-specific hierarchical structure.
# 26_Gender_specific_optimal_combination_reconciliation.R: calculate reconciled interval scores for the multivariate forecasting method with the optimal combination method following the gender-specific hierarchical structure.
# 27_Gender_specific_MinT reconciliation.R: calculate reconciled interval scores for the multivariate forecasting method by the MinT method following the gender-specific hierarchical structure.
# 28_Gender_specific_summing_matrix.R: construct summing matrix using forecast exposure to risk following the gender-specific hierarchical structure.
# 29_Gender_specific_new_reconciliation_structure.R: (Simplified version of 22) Reconcile point forecasts following the gender-specific summing matrix defined in 28.
