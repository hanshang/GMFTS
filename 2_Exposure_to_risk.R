...
pop_fore_fun <- function(top, bottom, year_horizon)
{
    ...
    for(year in 28:42)
    {
        ...
        Japan_pop_fore[,1,(year-27)] =  forecast(auto.arima(ratio), h = year_horizon)$mean
        
        for(ij in 2:year_horizon)
        {
            # x in year t is x+1 in year t+1
      
            Japan_pop_fore[ij,2:101,(year-27)] = Japan_pop_fore[ij-1,1:100,(year-27)]   
        }
    }
...
for(iw in 2:48)
{
    assign(pop_ratio_P_F_to_T[iw-1], pop_fore_fun(top = get(state[iw])$pop$female, bottom = Japan$pop$total, year_horizon = 15))
    assign(pop_ratio_P_M_to_T[iw-1], pop_fore_fun(top = get(state[iw])$pop$male,   bottom = Japan$pop$total, year_horizon = 15))
}
...
for(iw in 2:48)
{
    assign(pop_ratio_P_F_to_F[iw-1], pop_fore_fun(top = get(state[iw])$pop$female, bottom = Japan$pop$female, year_horizon = 15))
    assign(pop_ratio_P_M_to_M[iw-1], pop_fore_fun(top = get(state[iw])$pop$male,   bottom = Japan$pop$male, year_horizon = 15))
}
...
assign(pop_ratio_P_F_to_R1_T, pop_fore_fun(top = get(state[2])$pop$female, bottom = mfts_R1$pop$total, year_horizon = 15))
for(iw in 2:7)
{
    assign(pop_ratio_P_F_to_R2_T[iw-1], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R2$pop$total, year_horizon = 15))
}         
for(iw in 8:14)
{
    assign(pop_ratio_P_F_to_R3_T[iw-7], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R3$pop$total, year_horizon = 15))
}
for(iw in 15:23)
{
    assign(pop_ratio_P_F_to_R4_T[iw-14], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R4$pop$total, year_horizon = 15))
}
for(iw in 24:30)
{
    assign(pop_ratio_P_F_to_R5_T[iw-23], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R5$pop$total, year_horizon = 15))
}
for(iw in 31:35)
{
    assign(pop_ratio_P_F_to_R6_T[iw-30], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R6$pop$total, year_horizon = 15))
}
for(iw in 36:39)
{
    assign(pop_ratio_P_F_to_R7_T[iw-35], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R7$pop$total, year_horizon = 15))
}
for(iw in 40:47)
{
    assign(pop_ratio_P_F_to_R8_T[iw-39], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R8$pop$total, year_horizon = 15))
}
...
assign(pop_ratio_P_M_to_R1_T, pop_fore_fun(top = get(state[2])$pop$male, bottom = mfts_R1$pop$total, year_horizon = 15))
for(iw in 2:7)
{
    assign(pop_ratio_P_M_to_R2_T[iw-1], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R2$pop$total, year_horizon = 15))
}         
for(iw in 8:14)
{
    assign(pop_ratio_P_M_to_R3_T[iw-7], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R3$pop$total, year_horizon = 15))
}
for(iw in 15:23)
{
    assign(pop_ratio_P_M_to_R4_T[iw-14], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R4$pop$total, year_horizon = 15))
}
for(iw in 24:30)
{
    assign(pop_ratio_P_M_to_R5_T[iw-23], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R5$pop$total, year_horizon = 15))
}
for(iw in 31:35)
{
    assign(pop_ratio_P_M_to_R6_T[iw-30], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R6$pop$total, year_horizon = 15))
}
for(iw in 36:39)
{
    assign(pop_ratio_P_M_to_R7_T[iw-35], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R7$pop$total, year_horizon = 15))
}
for(iw in 40:47)
{
    assign(pop_ratio_P_M_to_R8_T[iw-39], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R8$pop$total, year_horizon = 15))
}
...
assign(pop_ratio_P_F_to_R1_F, pop_fore_fun(top = get(state[2])$pop$female, bottom = mfts_R1$pop$female, year_horizon = 15))
for(iw in 2:7)
{
    assign(pop_ratio_P_F_to_R2_F[iw-1], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R2$pop$female, year_horizon = 15))
}         
for(iw in 8:14)
{
    assign(pop_ratio_P_F_to_R3_F[iw-7], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R3$pop$female, year_horizon = 15))
}
for(iw in 15:23)
{
    assign(pop_ratio_P_F_to_R4_F[iw-14], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R4$pop$female, year_horizon = 15))
}
for(iw in 24:30)
{
    assign(pop_ratio_P_F_to_R5_F[iw-23], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R5$pop$female, year_horizon = 15))
}
for(iw in 31:35)
{
    assign(pop_ratio_P_F_to_R6_F[iw-30], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R6$pop$female, year_horizon = 15))
}
for(iw in 36:39)
{
    assign(pop_ratio_P_F_to_R7_F[iw-35], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R7$pop$female, year_horizon = 15))
}
for(iw in 40:47)
{
    assign(pop_ratio_P_F_to_R8_F[iw-39], pop_fore_fun(top = get(state[(iw+1)])$pop$female, bottom = mfts_R8$pop$female, year_horizon = 15))
}
...
assign(pop_ratio_P_M_to_R1_M, pop_fore_fun(top = get(state[2])$pop$male, bottom = mfts_R1$pop$male, year_horizon = 15))
for(iw in 2:7)
{
    assign(pop_ratio_P_M_to_R2_M[iw-1], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R2$pop$male, year_horizon = 15))
}         
for(iw in 8:14)
{
    assign(pop_ratio_P_M_to_R3_M[iw-7], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R3$pop$male, year_horizon = 15))
}
for(iw in 15:23)
{
    assign(pop_ratio_P_M_to_R4_M[iw-14], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R4$pop$male, year_horizon = 15))
}
for(iw in 24:30)
{
    assign(pop_ratio_P_M_to_R5_M[iw-23], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R5$pop$male, year_horizon = 15))
}
for(iw in 31:35)
{
    assign(pop_ratio_P_M_to_R6_M[iw-30], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R6$pop$male, year_horizon = 15))
}
for(iw in 36:39)
{
    assign(pop_ratio_P_M_to_R7_M[iw-35], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R7$pop$male, year_horizon = 15))
}
for(iw in 40:47)
{
    assign(pop_ratio_P_M_to_R8_M[iw-39], pop_fore_fun(top = get(state[(iw+1)])$pop$male, bottom = mfts_R8$pop$male, year_horizon = 15))
}
...
for(iw in 2:48)
{
    assign(pop_ratio_P_F_to_P_T[iw-1], pop_fore_fun(top = get(state[iw])$pop$female, bottom = get(state[iw])$pop$total, year_horizon = 15))
    assign(pop_ratio_P_M_to_P_T[iw-1], pop_fore_fun(top = get(state[iw])$pop$male,   bottom = get(state[iw])$pop$total, year_horizon = 15))          
}
